...
import { createStoryAction } from "@/app/actions/invitation/story";
import { toast } from "sonner";
import ButtonDndForm from "../button-dnd-form";

const MAX_MILESTONES = 3;
const STORIES_DEFAULT_VALUES = {
  stories: [
    {
      id: nanoid(),
      date: "",
...
export default function StoryCreateForm() {
  ...
  const form = useForm<StoriesFormType>({
    ...
    defaultValues: STORIES_DEFAULT_VALUES,
  });
  ...
  const onSubmit = async (data: StoriesFormType) => {
    ...
    const result = await createStoryAction(finalData);
    ...
    if (result.success) {
      toast.success("Story information created successfully!");
